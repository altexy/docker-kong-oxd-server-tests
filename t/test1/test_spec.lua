...
test("kong test", function()
    ...
    -- create a Sevice
    local res, err = sh_ex(
        [[curl -i -X POST --url http://localhost:8001/services/ --data 'name=test1-service' --data 'url=http://backend']]
    )

    -- create a Route
    local res, err = sh_ex(
        [[curl -i -X POST  --url http://localhost:8001/services/test1-service/routes --data 'hosts[]=backend.com']]
    )

    -- test it works
    local res, err = sh_ex([[curl -i -X GET --url http://localhost:8000/ --header 'Host: backend.com']])

    -- enable plugin for the Service
    local res, err = sh_ex([[
curl -i -X POST  --url http://localhost:8001/services/test1-service/plugins/  --data 'name=test1' \
