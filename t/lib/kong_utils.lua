...
_M.kong_postgress_custom_plugins = function(opts)
    ...
    ctx.finalizeres[#ctx.finalizeres + 1] = function()
        sh("docker stop  ", ctx.postgress_id)
        sh("docker rm -v  ", ctx.postgress_id)
    end
...
_M.backend = function(image)
    ...
    ctx.finalizeres[#ctx.finalizeres + 1] = function()
        sh("docker stop  ", ctx.backend_id)
        sh("docker rm -v  ", ctx.backend_id)
    end
...
_M.oxd_mock = function(model, image)
    ...
    ctx.finalizeres[#ctx.finalizeres + 1] = function()
        --sh("docker logs  ", ctx.oxd_id)
        sh("docker stop  ", ctx.oxd_id)
        sh("docker rm -v  ", ctx.oxd_id)
    end
